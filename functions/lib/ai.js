...
exports.buildFamilyProfileContext = buildFamilyProfileContext;
...
function buildFamilyProfileContext(familyProfile, members) {
    ...
    if (fp.monthlyHouseholdExpenses != null) {
        lines.push(`Emergency fund target (${emergencyMonths} months): ₹${formatInr(fp.monthlyHouseholdExpenses * emergencyMonths)}`);
    }
    ...
    if (fp.dependents && fp.dependents.length > 0) {
        ...
        for (const dep of fp.dependents) {
            ...
            lines.push(`- ${dep.name ?? "Unknown"} (${relationshipLabel(dep.relationship)}${age != null ? `, age ${age}` : ""})`);
        }
    }
    if (fp.loans && fp.loans.length > 0) {
        lines.push("\nLoans:");
        for (const loan of fp.loans) {
            lines.push(`- ${loanTypeLabel(loan.type)}: EMI ₹${formatInr(loan.emi ?? 0)}, ${loan.remainingMonths ?? 0} months left` +
    ...
    if (members && members.length > 0) {
        ...
        for (const member of members) {
            ...
            if (p.monthlyIncome != null)
                parts.push(`income ₹${formatInr(p.monthlyIncome)}`);
            if (p.monthlyExpenses != null)
                parts.push(`expenses ₹${formatInr(p.monthlyExpenses)}`);
            if (p.employmentType)
                parts.push(p.employmentType);
            if (p.riskAppetite)
                parts.push(`risk: ${p.riskAppetite}`);
            if (p.retirementAge)
                parts.push(`retire at ${p.retirementAge}`);
            lines.push(`- ${parts.join(", ")}`);
        }
    }
    const hasFamilyData = fp.monthlyHouseholdIncome != null ||
...
async function generateAINotification(params) {
    ...
    const blockedGoals = recentGoalIds.length > 0 ? recentGoalIds.join(", ") : "none";
    const recentSummary = params.recentNotifications.length === 0
        ? "None"
        : params.recentNotifications
            .map((n, i) => `${i + 1}. Goal ID: ${n.relatedGoalId ?? "portfolio-wide"}\n   Title: ${n.title}\n   Action: ${n.suggestedAction}\n   Body: ${n.body}`)
            .join("\n");
